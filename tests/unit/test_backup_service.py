...
class TestBackupService:
    ...
    def test_backup_postgres_runs_pg_dump(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(backup, "settings", mock_settings)
        monkeypatch.setattr(
            backup, "get_backup_dir", lambda date_folder=True: str(tmp_path)
        )
    ...
    def test_backup_qdrant_calls_snapshot_and_tars_storage(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            backup.tarfile, "open", MagicMock(return_value=fake_context)
        )
    ...
    def test_encrypt_and_decrypt_file_using_gpg_mock(self, tmp_path, monkeypatch):
        ...
        gpg_instance.decrypt_file.return_value = MagicMock(
            ok=True, status="decrypted", data=b"restored-bytes"
        )
    ...
    def test_restore_postgres_in_memory_calls_psql(self, tmp_path, monkeypatch):
        ...
        gpg_instance = MagicMock()
        gpg_instance.decrypt_file.return_value = MagicMock(
            ok=True, status="decrypted", data=b"SELECT 1;"
        )
        ...
        monkeypatch.setattr(
            backup.subprocess, "Popen", MagicMock(return_value=process_mock)
        )
    ...
    def test_restore_tar_in_memory_extracts_files(self, tmp_path, monkeypatch):
        ...
        gpg_instance = MagicMock()
        gpg_instance.decrypt_file.return_value = MagicMock(
            ok=True, status="decrypted", data=tar_bytes.getvalue()
        )
        monkeypatch.setattr(backup.gnupg, "GPG", MagicMock(return_value=gpg_instance))

        extract_path = tmp_path / "extract"
        backup.restore_tar_in_memory(
            str(encrypted_path), "passphrase", str(extract_path)
        )
    ...
    def test_wait_for_postgres_ready_returns_when_connected(self, monkeypatch):
        conn_mock = MagicMock()
        monkeypatch.setattr(
            backup.psycopg2, "connect", MagicMock(return_value=conn_mock)
        )
    ...
    def test_wait_for_qdrant_ready_returns_on_http_200(self, monkeypatch):
        ...
        monkeypatch.setattr(
            backup.requests, "get", MagicMock(return_value=mock_response)
        )
        mock_settings = MagicMock()
        mock_settings.QDRANT_URL = "http://localhost"
        monkeypatch.setattr(backup, "settings", mock_settings)
    ...
    def test_restore_backups_dispatches_to_restore_functions(
        ...
        restore_tar.assert_any_call(
            str(backup_date_dir / "data_backup_1.tar.gz.gpg"),
            "secret",
            str(tmp_path / "data"),
        )
        restore_tar.assert_any_call(
            str(backup_date_dir / "qdrant_backup_1.tar.gz.gpg"),
            "secret",
            "/qdrant/storage",
        )
    ...
    def test_main_performs_backups_and_cleanup(self, monkeypatch):
        ...
        monkeypatch.setattr(
            backup, "backup_postgres", MagicMock(return_value="postgres.sql")
        )
        monkeypatch.setattr(
            backup, "backup_data", MagicMock(return_value="data.tar.gz")
        )
        monkeypatch.setattr(
            backup, "backup_qdrant", MagicMock(return_value="qdrant.tar.gz")
        )
        monkeypatch.setattr(
            backup,
            "encrypt_file",
            MagicMock(
                side_effect=["postgres.sql.gpg", "data.tar.gz.gpg", "qdrant.tar.gz.gpg"]
            ),
        )
    ...
    def test_backup_postgres_error(self, tmp_path, monkeypatch):
        ...
        mock_run = MagicMock(
            side_effect=backup.subprocess.CalledProcessError(1, "pg_dump")
        )
        monkeypatch.setattr(backup.subprocess, "run", mock_run)
    ...
    def test_decrypt_file_not_ok(self, tmp_path, monkeypatch):
        ...
        gpg_instance = MagicMock()
        gpg_instance.decrypt_file.return_value = MagicMock(
            ok=False, status="Decryption failed status"
        )
        gpg_class = MagicMock(return_value=gpg_instance)
        monkeypatch.setattr(backup.gnupg, "GPG", gpg_class)
    ...
    def test_restore_postgres_in_memory_drop_recreate_error(
        self, tmp_path, monkeypatch
    ):
        mock_settings = MagicMock()
        mock_settings.POSTGRES_HOST = "localhost"
        ...
        process_mock.communicate.return_value = (
            b"",
            b"failed to drop recreate database",
        )
        monkeypatch.setattr(
            backup.subprocess, "Popen", MagicMock(return_value=process_mock)
        )
    ...
    def test_restore_postgres_in_memory_psql_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            backup.subprocess, "Popen", MagicMock(side_effect=popen_side_effect)
        )
    ...
    def test_wait_for_postgres_ready_timeout(self, monkeypatch):
        monkeypatch.setattr(
            backup.psycopg2,
            "connect",
            MagicMock(side_effect=Exception("DB not ready yet")),
        )
    ...
    def test_wait_for_qdrant_ready_exception(self, monkeypatch):
        monkeypatch.setattr(
            backup.requests, "get", MagicMock(side_effect=Exception("Network error"))
        )
        monkeypatch.setattr(backup.time, "sleep", lambda x: None)
        mock_settings = MagicMock()
        mock_settings.QDRANT_URL = "http://localhost"
    ...
    def test_main_generic_error(self, monkeypatch):
        ...
        monkeypatch.setattr(
            backup,
            "backup_postgres",
            MagicMock(side_effect=Exception("Postgres failed")),
        )
    ...
    def test_cleanup_old_backups_skips_non_dir_and_decrypted(
        self, tmp_path, monkeypatch
    ):
        base_dir = tmp_path / "cache" / "backups"
        base_dir.mkdir(parents=True)
    ...
    def test_cleanup_old_backups_error(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(Exception):
            backup.cleanup_old_backups(
                days=7, base_dir=str(tmp_path / "non_existent_directory_error")
            )
    ...
    def test_restore_tar_in_memory_clears_subdirectory(self, tmp_path, monkeypatch):
        ...
        backup.restore_tar_in_memory(
            str(encrypted_path), "passphrase", str(extract_path2)
        )
    ...
    def test_restore_tar_in_memory_clears_app_data(self, tmp_path, monkeypatch):
        ...
        gpg_instance = MagicMock()
        gpg_instance.decrypt_file.return_value = MagicMock(
            ok=True, status="decrypted", data=b"fake-tar-data"
        )
        monkeypatch.setattr(backup.gnupg, "GPG", MagicMock(return_value=gpg_instance))
        ...
        monkeypatch.setattr(
            backup.tarfile, "open", MagicMock(return_value=fake_tar_ctx)
        )
        ...
        def exists_mock(path):
            return (
                True
                if str(path).replace("\\", "/") in ["/app", "/app/data"]
        ...
        def isdir_mock(path):
            return (
                True
                if str(path).replace("\\", "/") == "/app/data/sub"
                else orig_isdir(path)
            )
    ...
    def test_run_as_main(self, tmp_path, monkeypatch):
        ...
        client = MagicMock()
        client.get_collections.return_value = SimpleNamespace(
            collections=[SimpleNamespace(name="col")]
        )
        ...
        def mock_open(file, mode="r", *args, **kwargs):
            if any(
                term in str(file)
                for term in ["postgres_backup", "data_backup", "qdrant_backup"]
            ):
