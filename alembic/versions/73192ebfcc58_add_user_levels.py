...
down_revision: Union[str, Sequence[str], None] = '944912616179'
...
def upgrade() -> None:
    ...
    user_level_enum = sa.Enum('LEVEL_01', 'LEVEL_02', 'LEVEL_03', 'LEVEL_04', 'LEVEL_05', name='userlevel')
    user_level_enum.create(op.get_bind(), checkfirst=True)
    
    # Add columns to users table
    op.add_column('users', sa.Column('level', user_level_enum, nullable=False, server_default='LEVEL_01'))
    ...
    op.execute(f"UPDATE users SET token_budget = {settings.TOKEN_BUDGET_LEVEL_01} WHERE token_budget IS NULL")
