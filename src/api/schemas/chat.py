...
class ChatCreate(BaseModel):
    ...
    title: str | None = Field(
        ...
        min_length=1,
        max_length=200,
        description="Chat title (optional, will generate placeholder if not provided)",
    )
...
class SendMessageResponse(BaseModel):
    ...
    sources: list[dict] | None = Field(
        None, description="Retrieved chunks used for RAG"
    )
...
class ChatModelUpdate(BaseModel):
    ...
    llm_model: str | None = Field(
        None, description="LLM model name (e.g., 'llama3.1:8b', 'gpt-4')"
    )
    llm_provider: str | None = Field(
        None, description="LLM provider (ollama, openai, gemini)"
    )
...
class LLMModelInfo(BaseModel):
    ...
    cost_tier: int = Field(
        5, ge=0, le=9, description="Cost tier 0-9 (0=cheapest, 9=most expensive)"
    )
