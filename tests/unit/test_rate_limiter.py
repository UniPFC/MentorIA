...
class TestSimpleRateLimiter:
    ...
    def test_is_blocked_under_limit(self):
        ...
        for i in range(2):
            limiter.attempts[key].append(datetime.now(timezone.utc))
    ...
    def test_record_attempt_new_key(self):
        ...
        assert limiter.attempts[key][0] <= datetime.now(timezone.utc)
    ...
    def test_record_attempt_existing_key(self):
        ...
        limiter.attempts[key] = [datetime.now(timezone.utc) - timedelta(minutes=1)]
