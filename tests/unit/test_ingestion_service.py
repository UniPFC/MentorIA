...
class TestChunkIngestionService:
    ...
    def test_detect_encoding_utf8(self, mock_embedding_engine, mock_qdrant_manager):
        csv_content = b"pergunta,resposta\nQ1,A1\nQ2,A2"
    ...
    def test_ingest_chunks_progress_callback_exception(
        ...
        point_ids, total = service.ingest_chunks(
            sample_chat_type.id, chunks, db_session, on_progress=failing_progress
        )
    ...
    def test_ingest_chunks_exception(
        ...
        with pytest.raises(Exception, match="Embedding failed"):
            service.ingest_chunks(sample_chat_type.id, chunks, db_session)
