...
class TestBackgroundServiceExtended:
    ...
    def test_process_ingestion_job_success_flow(
        self, mock_provider, mock_qdrant, mock_ingestion, mock_session_local,
        mock_db_session, sample_ingestion_job, mock_ingestion_service
    ):
        """Test successful ingestion job processing"""
        # Setup mocks
        mock_db_session.query.return_value.filter.return_value.first.return_value = sample_ingestion_job
    ...
    def test_process_ingestion_job_with_error(
        ...
        mock_ingestion_service.ingest_from_file.side_effect = Exception("Ingestion failed")
