...
class TestChatTypeFavoriteRepository:
    ...
    def test_get_by_user_and_chat_type_found(
        self,
        favorite_repo: ChatTypeFavoriteRepository,
        db_session: Session,
        sample_user: User,
        sample_chat_type: ChatType,
    ):
        favorite = ChatTypeFavorite(
            user_id=sample_user.id, chat_type_id=sample_chat_type.id
        )
        db_session.add(favorite)
        db_session.commit()

        result = favorite_repo.get_by_user_and_chat_type(
            sample_user.id, sample_chat_type.id
        )
    ...
    def test_get_by_user_and_chat_type_not_found(
        self, favorite_repo: ChatTypeFavoriteRepository, sample_user: User
    ):
        non_existent_chat_type_id = uuid4()
        result = favorite_repo.get_by_user_and_chat_type(
            sample_user.id, non_existent_chat_type_id
        )
    ...
    def test_get_user_favorite_ids(
        self,
        favorite_repo: ChatTypeFavoriteRepository,
        db_session: Session,
        sample_user: User,
    ...
    def test_get_user_favorite_ids_empty(
        self, favorite_repo: ChatTypeFavoriteRepository
    ):
        non_existent_user_id = uuid4()
    ...
    def test_delete(
        ...
        deleted = (
            db_session.query(ChatTypeFavorite)
            .filter(ChatTypeFavorite.id == favorite_id)
            .first()
        )
        assert deleted is None

    def test_delete_by_user_and_chat_type_success(
        self,
        favorite_repo: ChatTypeFavoriteRepository,
        db_session: Session,
        sample_user: User,
        ...
        result = favorite_repo.delete_by_user_and_chat_type(
            sample_user.id, sample_chat_type.id
        )
    ...
    def test_delete_by_user_and_chat_type_not_found(
        self, favorite_repo: ChatTypeFavoriteRepository, sample_user: User
    ):
        non_existent_chat_type_id = uuid4()
        result = favorite_repo.delete_by_user_and_chat_type(
