...
def verify_admin_slug(admin_slug: str = Path(..., description="Admin slug for authentication")):
    ...
    if admin_slug != settings.ADMIN_SLUG:
        raise HTTPException(status_code=403, detail="You're not authorized to access this page")
    return True
...
async def restore_backup(
    ...
    try:
        ...
        formatted_date = f"{request.date_str[:2]}/{request.date_str[2:4]}/{request.date_str[4:8]}"
...
async def delete_backup(
    ...
    try:
        ...
        if not os.path.exists(backup_date_dir):
            raise HTTPException(status_code=404, detail=f"Backup folder not found: {date_str}")
        ...
        return BackupResponse(
            success=True,
            message=f"Backup folder {date_str} deleted successfully"
        )
