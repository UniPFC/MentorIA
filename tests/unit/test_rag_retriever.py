...
class TestKnowledgeRetriever:
    ...
    def test_search_chunks(self, mock_qdrant_manager, mock_embedding_engine):
        retriever = KnowledgeRetriever(
            qdrant_manager=mock_qdrant_manager, embedding_engine=mock_embedding_engine
        )

        chat_type_id = uuid4()
        query = "What is AI?"

        chunks = retriever.search(chat_type_id=chat_type_id, query=query, limit=5)
    ...
    def test_search_with_score_threshold(
        self, mock_qdrant_manager, mock_embedding_engine
    ):
        retriever = KnowledgeRetriever(
            qdrant_manager=mock_qdrant_manager, embedding_engine=mock_embedding_engine
        )
        ...
        chunks = retriever.search(
            chat_type_id=chat_type_id, query=query, limit=5, score_threshold=0.9
        )
    ...
    def test_search_many_deduplication(
        self, mock_qdrant_manager, mock_embedding_engine
    ):
        ...
        chunks = retriever.search_many(
            chat_type_id=chat_type_id, queries=["query1", "query2"], limit_per_query=5
        )
