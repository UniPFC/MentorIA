...
def upgrade() -> None:
    ...
    op.add_column(
        "users", sa.Column("pagarme_customer_id", sa.String(length=255), nullable=True)
    )
    op.add_column(
    ...
    op.create_index(
        op.f("ix_users_pagarme_customer_id"),
        "users",
        ["pagarme_customer_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_users_subscription_id"), "users", ["subscription_id"], unique=False
    )
