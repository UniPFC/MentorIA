...
class TestChatTypeRepository:
    ...
    def test_get_by_id_found(
        self, chat_type_repo: ChatTypeRepository, sample_chat_type: ChatType
    ):
    ...
    def test_delete(
        self,
        chat_type_repo: ChatTypeRepository,
        db_session: Session,
    ...
    def test_search_without_filters(
        self, chat_type_repo: ChatTypeRepository, db_session: Session, sample_user: User
    ):
        ct1 = ChatType(
            id=uuid4(),
            name="Public Type 1",
    ...
    def test_search_with_is_public_filter(
        self, chat_type_repo: ChatTypeRepository, db_session: Session, sample_user: User
    ):
        ct1 = ChatType(
            id=uuid4(),
        ...
        chat_types, total = chat_type_repo.search(
            is_public=True, user_id=sample_user.id
        )

        assert total == 1
        assert chat_types[0].is_public is True

    def test_search_with_owner_filter(
        self, chat_type_repo: ChatTypeRepository, db_session: Session, sample_user: User
    ):
        other_user = User(
            id=uuid4(),
        ...
        chat_types, total = chat_type_repo.search(
            owner_id=sample_user.id, user_id=sample_user.id
        )
    ...
    def test_search_with_pagination(
        ...
        chat_types, total = chat_type_repo.search(
            user_id=sample_user.id, skip=2, limit=2
        )

        assert total == 5
        assert len(chat_types) == 2

    def test_search_security_filter(
        self, chat_type_repo: ChatTypeRepository, db_session: Session, sample_user: User
    ):
        other_user = User(
            id=uuid4(),
    ...
    def test_list_user_available_empty_favorites(
        self, chat_type_repo: ChatTypeRepository, db_session: Session, sample_user: User
    ):
        ct = ChatType(
            id=uuid4(),
    ...
    def test_list_by_ids_with_pagination(
        self, chat_type_repo: ChatTypeRepository, db_session: Session, sample_user: User
    ):
        ids = []
        for i in range(5):
    ...
    def test_list_user_available_with_filters(
        self, chat_type_repo: ChatTypeRepository, db_session: Session, sample_user: User
    ):
        ct1 = ChatType(
            id=uuid4(),
        ...
        chat_types, total = chat_type_repo.list_user_available(
            sample_user.id, [], owner_id=sample_user.id
        )
        assert total == 2
