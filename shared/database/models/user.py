...
class User(Base):
    ...
    _email = Column(String(500), unique=True, nullable=False, index=True)  # Aumentado para dados criptografados
    ...
    @property
    def email(self) -> str:
        """Retorna email decriptado (sobrescreve a coluna)"""
    ...
    def email(self, value: str):
        ...
        if value:
            try:
                # Tenta decriptar para ver se já está criptografado
                decrypt_sensitive_data(value)
                # Se conseguiu decriptar, já está criptografado
                self._email = value
            except:
                # Se falhou, precisa criptografar
